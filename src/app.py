...
# ----------------------------------------------------------
# ----------------------------------------------------------
# LOAD MODEL (robust: won't crash server if unpickling fails)
# ----------------------------------------------------------
...
def make_dummy_model():
    # safe fallback model: always returns "no CKD" (0) and zero probability
    class DummyModel:
        def predict(self, X):
            # X might be a 2D array-like
            try:
                n = len(X)
            except Exception:
                n = 1
        ...
        def predict_proba(self, X):
            try:
                ...
                n = 1
            # return [prob_normal, prob_ckd] shape
            return [[1.0, 0.0] for _ in range(n)]
    return DummyModel()

try:
    # Try to load real model artifacts
    model = joblib.load(os.path.join(MODEL_DIR, "ckd_model.pkl"))
    scaler = joblib.load(os.path.join(MODEL_DIR, "scaler.pkl"))
    feature_names = joblib.load(os.path.join(MODEL_DIR, "feature_names.pkl"))
    print("✅ Loaded model, scaler, feature names.")
except Exception as e:
    # If something goes wrong (missing files or missing libs), fall back to dummy objects
    import traceback
    print("❌ Warning: failed to load model artifacts. Falling back to dummy model. Error:")
    traceback.print_exc()
    model = make_dummy_model()
    # scaler fallback: identity transform
    class _IdentityScaler:
        def transform(self, X):
            return X
    scaler = _IdentityScaler()
    # if feature_names couldn't be loaded, create a minimal list to avoid KeyErrors in form parsing
    if not feature_names:
        feature_names = ["GFR"]  # minimal placeholder; app will still run

# note: application continues even if model loading failed.
